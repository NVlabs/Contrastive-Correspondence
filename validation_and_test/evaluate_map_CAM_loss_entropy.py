...
from model import scot_CAM_loss, geometry, evaluation, util
...
def run(datapath, benchmark, backbone, thres, alpha, hyperpixel,
    ...
    if model is None:
        model = scot_CAM_loss.SCOT_CAM(backbone, hyperpixel, benchmark, device,
                                       args.cam, args.num_classes,
                                       args.modelpath, args.temp,
                                       args.icycle_lw, args.entropy_lw)
    else:
    ...
    for idx, data in enumerate(dataloader):
        ...
        data['src_img'], data['src_kps'], data['src_intratio'] = util.resize(data['src_img'], data['src_kps'][0], 224)
        data['trg_img'], data['trg_kps'], data['trg_intratio'] = util.resize(data['trg_img'], data['trg_kps'][0], 224)
        ...
        with torch.no_grad():
            score = model(data['src_img'], data['trg_img'], args.sim, args.exp1, args.exp2, args.eps, args.classmap, data['src_bbox'], data['trg_bbox'], data['src_mask'], data['trg_mask'], backbone, data['crop'], data['crop_gt'],)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--icycle_lw', type=float, default=0.0001, help='image cycle loss weight')
    parser.add_argument('--entropy_lw', type=float, default=0.0001, help='entropy loss weight')
