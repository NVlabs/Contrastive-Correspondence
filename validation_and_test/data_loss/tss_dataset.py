...
from data.flow import read_flo_file
...
class TSSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'src_img': image_A, 'trg_img': image_B, 'src_size': im_size_A, 'trg_size': im_size_B, 'flow_path': flow_path}
        
        if self.transform:
            sample = self.transform(sample)

        sample['src_img'] = sample['src_img'].cuda()
        sample['trg_img'] = sample['trg_img'].cuda()
