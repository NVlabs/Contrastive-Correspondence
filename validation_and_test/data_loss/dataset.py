...
class CorrespondenceDataset(Dataset):
    ...
    def __init__(self, benchmark, datapath, thres, device, split):
        ...
        self.crop_path = os.path.join(base_path, self.metadata[benchmark][5])
    ...
    def __getitem__(self, idx):
        ...
        # crop and crop gt
        if os.path.exists(self.crop_path) and os.listdir(self.crop_path):
            if self.benchmark == 'pfpascal' or self.benchmark == 'pfwillow':
                sample['crop_name'] = os.path.splitext(self.src_imnames[idx])[0] + '_crop.pt'
                sample['crop_gt_name'] = os.path.splitext(self.src_imnames[idx])[0] + '_crop_gt.pt'
                sample['crop'], sample['crop_gt'] = self.get_crop_and_gt(sample['crop_name'], sample['crop_gt_name'], idx)
            elif self.benchmark == 'spair':
                src_pre_name =  os.path.splitext(sample['src_imname'])[0]
                trg_pre_name =  os.path.splitext(sample['trg_imname'])[0]
                sample['crop_name'] = os.path.join(self.crop_path,
                    '{:06d}-{}-{}_crop.gt'.format(idx+1, src_pre_name, trg_pre_name)
                )
                sample['crop_gt_name'] = os.path.join(self.crop_path,
    ...
    def get_crop_and_gt(self, crop_name, crop_gt_name, idx):
        crop = torch.load(os.path.join(self.crop_path, crop_name))
        crop_gt = torch.load(os.path.join(self.crop_path, crop_gt_name))
        return crop.to(self.device), crop_gt.to(self.device)
